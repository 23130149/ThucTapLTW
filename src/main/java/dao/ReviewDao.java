...
public class ReviewDao extends BaseDao {
        ...
        public List<Review> getAdminReviews(String keyword, Integer rating, String status) {
            StringBuilder sql = new StringBuilder("""
                    SELECT
                        ...
                        r.User_Id AS userId,
                        COALESCE(u.User_Name, u.Email, 'Khách hàng') AS userName,
                        COALESCE(p.Product_Name, 'Sản phẩm đã xoá') AS productName,
                        r.Rating AS rating,
                        r.Comment AS comment,
            ...
            if (hasKeyword) {
                sql.append("""
                        AND (
                            ...
                            OR COALESCE(u.User_Name, '') LIKE CONCAT('%', :keyword, '%')
                            OR COALESCE(u.Email, '') LIKE CONCAT('%', :keyword, '%')
                            OR COALESCE(p.Product_Name, '') LIKE CONCAT('%', :keyword, '%')
                        )
                        """);
            }
        ...
        public double getAdminAverageRating() {
            String sql = """
                    SELECT COALESCE(AVG(Rating), 0)
                    FROM reviews
                    """;
